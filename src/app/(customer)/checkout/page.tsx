...
export default function CheckoutPage() {
  ...
  // Sync or fetch cafeId
  useEffect(() => {
    if (cafeId) {
      setResolvedCafeId(cafeId);
      return;
    }
    // Fallback: fetch first cafe if not in cart
    setSlotsLoading(true);
    fetch("/api/cafes/default")
      .then((r) => r.json())
      .then((d) => {
        if (d.cafe_id) {
          setResolvedCafeId(d.cafe_id);
        } else {
          setSlotsError("Could not find cafe");
        }
      })
      .catch((err) => {
        console.error("Cafe fetch error:", err);
        setSlotsError("Failed to load cafe information");
      })
  ...
  useEffect(() => {
    if (!resolvedCafeId) {
      setSlotsError("No cafe selected");
      setSlotsLoading(false);
      return;
    }
    setSlotsLoading(true);
    setSlotsError("");
    fetch(`/api/slots?cafe_id=${resolvedCafeId}&date=${dateStr}`)
      .then((r) => {
      ...
      .then((d) => {
        if (!d.data || d.data.length === 0) {
          setSlotsError("No slots available. Admin may need to create slots.");
          setSlots([]);
        } else {
          setSlotsError("");
          setSlots(d.data);
      ...
      <div className="page-container pt-6">
        ...
        <section className="card p-5 mb-4">
          ...
          {slotsLoading ? (
            ...
            <div className="text-red-400 text-sm text-center py-4 px-3 bg-red-400/10 border border-red-400/20 rounded-lg flex items-center gap-2">
              <AlertCircle size={16} />
            ...
            <div className="grid grid-cols-3 gap-2 max-h-48 overflow-y-auto no-scrollbar">
              {slots.map((slot) => {
                const full =
                  slot.booked_count >= slot.max_orders || slot.is_blocked;
                return (
                  <button
                    key={slot.id}
                    disabled={full}
