...
async function getStats(cafeId: string) {
  ...
  const [todayOrders, monthOrders, pending, slotsCount] = await Promise.all([
    supabase
      .from("orders")
  ...
  return {
    ...
    today_revenue:
      todayOrders.data?.reduce((s, o) => s + Number(o.total_amount), 0) ?? 0,
    month_orders: monthOrders.data?.length ?? 0,
    month_revenue:
      monthOrders.data?.reduce((s, o) => s + Number(o.total_amount), 0) ?? 0,
    pending_orders: pending.count ?? 0,
    slots_count: slotsCount.count ?? 0,
  };
}
...
export default async function AdminDashboard() {
  ...
  return (
    <div>
      ...
      {/* Slot Initialization Alert */}
      {stats.slots_count === 0 && (
        <div className="mb-8 p-4 rounded-lg bg-amber-50 border border-amber-200 flex items-start justify-between">
          <div>
            <p className="font-semibold text-amber-900 mb-1 flex items-center gap-2">
              <AlertCircle size={16} /> No pickup slots configured
            </p>
            <p className="text-sm text-amber-800">
              Customers cannot place orders without available pickup slots.
              Create default time slots to enable ordering.
            </p>
          </div>
          <Link
            href="/admin/slots"
